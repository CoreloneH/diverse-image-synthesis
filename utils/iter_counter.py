...
class IterationCounter():
    def __init__(self, cfg, dataset_size):
        ...
        self.first_epoch = 1
        self.total_epochs = cfg.TRAIN.MAX_EPOCH
        self.epoch_iter = 0  # iter number within each epoch
        self.iter_record_path = os.path.join(self.cfg.checkpoints_dir, self.cfg.CONFIG_NAME, 'iter.txt')
        mkdir_p(os.path.join(self.cfg.checkpoints_dir, self.cfg.CONFIG_NAME))
        with open(self.iter_record_path,'w') as f:
            pass
        if cfg.TRAIN.FLAG and cfg.TRAIN.continue_train:
            try:
    ...
    def record_one_iteration(self):
        ...
        self.time_per_iter = (current_time - self.last_iter_time) / self.cfg.TRAIN.BATCH_SIZE
        self.last_iter_time = current_time
        self.total_steps_so_far += self.cfg.TRAIN.BATCH_SIZE
        self.epoch_iter += self.cfg.TRAIN.BATCH_SIZE
    ...
    def needs_saving(self):
        return (self.total_steps_so_far % self.cfg.save_latest_freq) < self.cfg.TRAIN.BATCH_SIZE

    def needs_printing(self):
        return (self.total_steps_so_far % self.cfg.print_freq) < self.cfg.TRAIN.BATCH_SIZE

    def needs_displaying(self):
        return (self.total_steps_so_far % self.cfg.display_freq) < self.cfg.TRAIN.BATCH_SIZE
